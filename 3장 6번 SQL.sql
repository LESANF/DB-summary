...
-> SELECT 극장.극장번호, COUNT(*) AS 고객수 FROM 극장, 예약 WHERE 극장.극장번호 = 예약.극장번호 AND 예약.날짜='2014-09-01' GROUP BY 극장.극장번호 

4. 2014년 9월 1일에 가장 많은 고객이 관람한 영화를 보이시오.
->SELECT 영화제목
   FROM 상영관, 예약
   WHERE 상영관.극장번호=예약.극장번호 AND 상영관.상영관번호 = 예약.상영관번호 AND 날짜='2014-09-01'
   GROUP BY 예약.극장번호, 예약.상영관번호, 영화제목
   HAVING COUNT(*) = (SELECT MAX(COUNT(영화제목))
                                    FROM 상영관, 예약
                                    WHERE 상영관.극장번호=예약.극장번호 AND 상영관.상영관번호=예약.상영관번호 AND 날짜 = '2014-09-01'
                                    GROUP BY 영화제목)
...
-> INSERT INTO 테이블이름[(속성리스트)] VALUES (값리스트);  [ 속성의 이름은 생략가능하나 값리스트는 순서대로 다넣어야함 ]
